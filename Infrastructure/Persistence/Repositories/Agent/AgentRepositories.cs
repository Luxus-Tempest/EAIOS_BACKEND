...
public interface IAgentRepository
{
    ...
    Task<PagedResult<Domain.Agent.Agent>> SearchAsync(string? search, AgentType? type, AgentStatus? status, AgentVisibility? visibility, int page, int pageSize, CancellationToken ct = default);
...
{
    public async Task<PagedResult<Domain.Agent.Agent>> SearchAsync(string? search, AgentType? type, AgentStatus? status, AgentVisibility? visibility, int page, int pageSize, CancellationToken ct = default)
    {
        var query = Set.AsQueryable();
        if (!string.IsNullOrWhiteSpace(search)) query = query.Where(a => a.Name.Contains(search) || a.DisplayName!.Contains(search));
...
public interface IAgentExecutionRepository
{
    ...
    Task<PagedResult<AgentExecution>> GetByAgentIdAsync(Guid agentId, int page, int pageSize, CancellationToken ct = default);
    Task<PagedResult<AgentExecution>> GetByUserIdAsync(Guid userId, int page, int pageSize, CancellationToken ct = default);
...
{
    public async Task<PagedResult<AgentExecution>> GetByAgentIdAsync(Guid agentId, int page, int pageSize, CancellationToken ct = default)
    {
        var query = Set.Where(e => e.AgentId == agentId).OrderByDescending(e => e.StartedAt);
        var total = await query.CountAsync(ct);
        var items = await query.Skip((page - 1) * pageSize).Take(pageSize).ToListAsync(ct);
        return new PagedResult<AgentExecution>(items, page, pageSize, total);
    }

    public async Task<PagedResult<AgentExecution>> GetByUserIdAsync(Guid userId, int page, int pageSize, CancellationToken ct = default)
    {
        var query = Set.Where(e => e.UserId == userId).OrderByDescending(e => e.StartedAt);
        var total = await query.CountAsync(ct);
        var items = await query.Skip((page - 1) * pageSize).Take(pageSize).ToListAsync(ct);
        return new PagedResult<AgentExecution>(items, page, pageSize, total);
    }
...
public interface IAgentMemoryRepository
{
    ...
    Task<IReadOnlyList<AgentMemory>> GetByAgentIdAsync(Guid agentId, Guid? userId, AgentMemoryType? type, CancellationToken ct = default);
...
public sealed class AgentMemoryRepository(EaiosDbContext db) : RepositoryBase<AgentMemory>(db), IAgentMemoryRepository
{
    ...
    public async Task<IReadOnlyList<AgentMemory>> GetByAgentIdAsync(Guid agentId, Guid? userId, AgentMemoryType? type, CancellationToken ct = default)
    {
        ...
        return (await q.OrderByDescending(m => m.ImportanceScore).ToListAsync(ct)).AsReadOnly();
    }
}
